...
const Navbar = ()=>{
  ...
  return (
    <Container className={`${isScrolled ? "bg-white/20 backdrop-blur-lg border-white/30 shadow-lg" : "bg-black"}  sticky top-0`}>
    <nav className=" w-full h-[70px] flex  lg:justify-center lg:items-center">
      <div className="w-full flex justify-between lg:justify-center  items-center">
        ...
        <div className="hidden lg:flex w-full justify-evenly items-center">
          { 
            links.map((item,i) => (<Link className="text-sm text-white uppercase  font-semibold" key={i} href={item.link}>{item.title}</Link>))
          }
        </div>
        <div className='lg:hidden p-0.5'>
          <Button className='border-[1px] border-primary-forground' onClick={()=> (console.log("I am button."))}  variant="secondary"  size="icon">
